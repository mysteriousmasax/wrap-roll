...
export default function AnalyticsPage() {
  ...
  const pettyCashTotal = pettyCashRows.reduce((sum, row) => sum + (Number(row.rate) || 0) * (Number(row.quantity) || 0), 0);
  const dailySalesTotal = salesSummaryRows.reduce((sum, row) => sum + (Number(row.quantity) || 0) * (Number(row.price) || 0), 0);
    ...
    <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <section className="space-y-4" aria-label="Daily operational summaries">
        <div className="flex items-end justify-between gap-4">
          <div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#ae002a]">Cash control</p><h2 className="mt-1 font-display text-xl font-bold text-[#1f1d1b]">Petty Cash Summary</h2><p className="mt-1 text-xs text-[#746e67]">Record small cash purchases and weekly approvals.</p></div>
          <WalletCards size={24} className="text-[#ae002a]" />
        </div>
        <Card className="border border-[#ebdccb] bg-white rounded-3xl p-5 shadow-sm">
          <div className="mb-4 grid grid-cols-1 gap-3 sm:grid-cols-3"><label className="text-xs font-bold text-[#554e46] sm:col-span-2">Week ending<input type="date" value={pettyCashMeta.weekEnding} onChange={(event) => setPettyCashMeta({ ...pettyCashMeta, weekEnding: event.target.value })} className="mt-1 w-full rounded-xl border border-[#ebdccb] bg-[#fffaf5] px-3 py-2 text-sm font-normal" /></label><div className="rounded-xl bg-[#faeee2] p-3"><p className="text-[10px] font-bold uppercase text-[#746e67]">Total cash</p><p className="mt-1 font-display text-xl font-bold text-[#ae002a]">{formatCurrency(pettyCashTotal)}</p></div></div>
        ...
        <div className="flex items-end justify-between gap-4 pt-3"><div><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#ae002a]">Stock movement</p><h2 className="mt-1 font-display text-xl font-bold text-[#1f1d1b]">Daily Sales Summary</h2><p className="mt-1 text-xs text-[#746e67]">Compare opening and closing stock by shift and item.</p></div><ClipboardList size={24} className="text-[#ae002a]" /></div>
        <Card className="border border-[#ebdccb] bg-white rounded-3xl p-5 shadow-sm"><div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><label className="text-xs font-bold text-[#554e46]">Shift<input value={salesSummaryMeta.shift} onChange={(event) => setSalesSummaryMeta({ ...salesSummaryMeta, shift: event.target.value })} placeholder="Morning / Afternoon / Evening" className="mt-1 w-full rounded-xl border border-[#ebdccb] bg-[#fffaf5] px-3 py-2 font-normal sm:w-64" /></label><div className="rounded-xl bg-[#f0f9f3] px-4 py-3 text-right"><p className="text-[10px] font-bold uppercase text-[#746e67]">Total sales</p><p className="font-display text-xl font-bold text-[#227653]">{formatCurrency(dailySalesTotal)}</p></div></div><div className="overflow-x-auto"><table className="w-full min-w-[1040px] border-collapse text-left text-xs"><thead className="bg-[#fbf6ee] text-[10px] uppercase tracking-wide text-[#746e67]"><tr><th className="border border-[#ebdccb] p-2">Date</th><th className="border border-[#ebdccb] p-2">Item</th><th className="border border-[#ebdccb] p-2">Qty</th><th className="border border-[#ebdccb] p-2">Opening stock</th><th className="border border-[#ebdccb] p-2">Closing stock</th><th className="border border-[#ebdccb] p-2">Price</th><th className="border border-[#ebdccb] p-2">Difference</th><th className="border border-[#ebdccb] p-2">Total</th><th className="border border-[#ebdccb] p-2">Remarks</th></tr></thead><tbody>{salesSummaryRows.map((row) => <tr key={row.id}><td className="border border-[#ebdccb] p-1"><input aria-label={`Sales date ${row.id}`} type="date" value={row.date} onChange={(event) => updateSalesSummaryRow(row.id, 'date', event.target.value)} className="w-32 bg-transparent p-1 outline-none" /></td><td className="border border-[#ebdccb] p-1"><input aria-label={`Sales item ${row.id}`} value={row.item} onChange={(event) => updateSalesSummaryRow(row.id, 'item', event.target.value)} className="w-36 bg-transparent p-1 outline-none" /></td>{['quantity', 'openingStock', 'closingStock', 'price'].map((field) => <td key={field} className="border border-[#ebdccb] p-1"><input aria-label={`${field} ${row.id}`} type="number" value={row[field]} onChange={(event) => updateSalesSummaryRow(row.id, field, event.target.value)} className="w-24 bg-transparent p-1 outline-none" /></td>)}<td className="border border-[#ebdccb] p-2 font-semibold text-[#554e46]">{dailyDifference(row)}</td><td className="border border-[#ebdccb] p-2 font-bold text-[#ae002a]">{formatCurrency((Number(row.quantity) || 0) * (Number(row.price) || 0))}</td><td className="border border-[#ebdccb] p-1"><input aria-label={`Sales remarks ${row.id}`} value={row.remarks} onChange={(event) => updateSalesSummaryRow(row.id, 'remarks', event.target.value)} className="w-full bg-transparent p-1 outline-none" /></td></tr>)}</tbody></table></div><div className="mt-4 grid grid-cols-1 gap-3 border-t border-[#eee4d5] pt-4 lg:grid-cols-3"><label className="text-xs font-bold text-[#554e46]">Checked by<input value={salesSummaryMeta.checkedBy} onChange={(event) => setSalesSummaryMeta({ ...salesSummaryMeta, checkedBy: event.target.value })} className="mt-1 w-full rounded-xl border border-[#ebdccb] px-3 py-2 font-normal" /></label><label className="text-xs font-bold text-[#554e46]">Approved by<input value={salesSummaryMeta.approvedBy} onChange={(event) => setSalesSummaryMeta({ ...salesSummaryMeta, approvedBy: event.target.value })} className="mt-1 w-full rounded-xl border border-[#ebdccb] px-3 py-2 font-normal" /></label><div><p className="text-xs font-bold text-[#554e46]">Wastage</p><div className="mt-1 grid grid-cols-2 gap-2">{salesSummaryMeta.wastage.map((value, index) => <input key={index} aria-label={`Wastage ${index + 1}`} value={value} onChange={(event) => setSalesSummaryMeta({ ...salesSummaryMeta, wastage: salesSummaryMeta.wastage.map((entry, entryIndex) => entryIndex === index ? event.target.value : entry) })} placeholder={`${index + 1}`} className="rounded-xl border border-[#ebdccb] px-3 py-2" />)}</div></div></div></Card>
      </section>
    </div>
  );
}
