          ...
          <div className={'overflow-hidden rounded-sm bg-[#f7f3ec] shadow-[0_18px_45px_rgba(0,0,0,0.28)] ' + (previewFormat === 'xlsx' ? 'ring-4 ring-[#227653]/20' : previewFormat === 'docx' ? 'ring-4 ring-[#2d5b9c]/20' : previewFormat === 'pptx' ? 'ring-4 ring-[#e37b22]/20' : 'ring-4 ring-[#ae002a]/10')}>
            ...
            <div className="space-y-5 bg-[#302e2c] p-3 sm:p-6">
              {Object.entries(previewGroups).map(([category, categoryItems], categoryIndex) => <section key={category} className="relative overflow-hidden bg-[#fffdf9] px-5 py-8 shadow-[0_8px_24px_rgba(0,0,0,0.16)] sm:px-10 sm:py-10">
                ...
                <div className="relative grid gap-x-7 gap-y-4 sm:grid-cols-2">
                  {categoryItems.slice(0, 10).map((item, itemIndex) => <article key={item.id} className="flex min-w-0 gap-2 border-b border-dotted border-[#c9bbae] pb-3"><div className="h-14 w-14 shrink-0 overflow-hidden bg-[#eaded3]"><img src={getSafeMenuImage(item.image)} alt="" className="h-full w-full object-cover" onError={(event) => { event.currentTarget.src = FALLBACK_MENU_IMAGE; }} /></div><div className="min-w-0 flex-1"><div className="flex items-start gap-2"><h4 className="line-clamp-1 flex-1 text-[11px] font-black uppercase leading-4 text-[#292522]">{item.name}</h4><span className="shrink-0 text-[11px] font-black text-[#ae002a]">{formatCurrency(item.price)}</span></div><p className="mt-1 line-clamp-2 text-[9px] leading-3 text-[#786a62]">{item.description || 'Freshly prepared at Wrap & Roll.'}</p>{item.popular && <span className="mt-1 inline-block text-[8px] font-bold uppercase tracking-wide text-[#b47a13]">★ Favourite</span>}</div></article>)}
                </div>
